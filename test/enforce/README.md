...
container (there is no host Go):
    ...
    docker run --rm -v "$PWD":/w -v beacon-gomodcache:/go/pkg/mod -w /w \
...
- **Integration leg** (`//go:build integration` files, run by the live
  ...
  *allowed* resource-absent kind — they skip cleanly when a Docker socket or a
  live external service is missing and run for real where those are present.
  Their count is capped at the committed budget in `skip-budget.txt`, a
  **ratchet**: removing a skip never breaks the build, and raising the budget is
  a reviewed change that must name the new resource-absent skip it admits.

beacon has no `//go:build integration` files yet (no live harness — tracked
Tier-B debt in `docs/TESTING.md`), so today both buckets are 0.
...
scoped to the **wiring** packages only (`internal/daemon`, `internal/watch`,
`internal/ingest`, `internal/delivery`, `internal/spool`) — the
socket/HTTP-event-to-effect layer where this suite's confirmed bugs actually
live. It is deliberately not a global percentage: a global number rewards
testing the cheap pure core and is satisfied while the wiring stays untouched.
See `coverage-floor.txt` for the per-package wiring-vs-core classification and
its reasoning. The floor is a ratchet: it may only rise. Edit a number upward
when coverage climbs; never downward to make a regression pass.

### `check-deadcode.sh` — the built-but-not-wired analyzer

Runs `golang.org/x/tools/cmd/deadcode` (pinned) rooted at the real `main`
(beacon is a daemon module, `cmd/beacon`, so `deadcode ./...` roots at `main`,
never `-test`). Any function unreachable from the production entrypoint is
reported and fails the build. This machine-checks the built-but-not-wired class
directly — the Billet dead-code lesson that every unit test passed over. Zero
tolerance. (Because the root is `main` and not `-test`, a test-only helper must
live in a `_test.go` file or use the shared out-of-module `core/runtime/
runtimetest` fake, never a production `.go` file, or it reads as unreachable.)
...
attestation match the tag being published exactly. beacon's `operator_run` is
`none` — beacon has no live harness yet, so nothing is operator-run and there is
nothing that can go stale; the gate stays in place for the day the harness lands
and a real external-service scenario is added.
